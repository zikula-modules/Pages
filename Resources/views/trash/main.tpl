    ...
    <ul>
        {foreach from=$property.subcategories item='category'}
            ...
            {if $modvars.ZConfig.shorturls}
                <li>
                    <a href="{route name='zikulapagesmodule_user_view' prop=$property.name cat=$category.path|replace:$property.rootcat.path:''}"
                       title="{$categorydesc}">{$categoryname}</a>
                    ({gt text='%s page' plural='%s pages' count=$category.count tag1=$category.count})
                </li>
            {else}
                <li><a href="{route name='zikulapagesmodule_user_view' prop=$property.name cat=$category.id}"
                       title="{$categorydesc}">{$categoryname}</a>
                    ({gt text='%s page' plural='%s pages' count=$category.count tag1=$category.count})
                </li>
            {/if}
