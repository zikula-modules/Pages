...
function zikulaPagesTriggerFormValidation() {
    zikulaPagesExecuteCustomValidationConstraints(editedObjectType, editedEntityId);
...
function zikulaPagesHandleFormSubmit(event) {
    if (triggerValidation) {
        zikulaPagesTriggerFormValidation();
        if (!editForm.get(0).checkValidity()) {
 ...
 */
function zikulaPagesInitEditForm(mode, entityId) {
    if (jQuery('.zikulapages-edit-form').length < 1) {
        return;
    }

    editForm = jQuery('.zikulapages-edit-form').first();
    ...
    editForm.submit(zikulaPagesHandleFormSubmit);
    ...
    if (jQuery('.field-editing-definition').length > 0) {
        jQuery('.field-editing-definition').each(function (index) {
            if ('user' === jQuery(this).data('field-type')) {
                ...
                zikulaPagesInitDateField(jQuery(this).data('field-name'));
            } else if ('upload' === jQuery(this).data('field-type')) {
                zikulaPagesInitUploadField(jQuery(this).data('field-name'));
            }
        });
    }
    if (jQuery('#formEditingDefinition').length > 0) {
        zikulaPagesInitEditForm(jQuery('#formEditingDefinition').data('mode'), jQuery('#formEditingDefinition').data('entityid'));
    }
});
